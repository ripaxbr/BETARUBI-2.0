...
WEBPLAY_URL = os.getenv("WEBPLAY_URL", "")
...
def youtube_search_channels(country_code, language):
    ...
    params = urlencode({"part":"snippet","type":"channel","maxResults":50,"order":"relevance","regionCode":country_code,"relevanceLanguage":language,"key":YT_KEY,"q":"official television news culture entertainment"})
...
def youtube_search_videos(country_code, language, term):
    if not YT_KEY: return []
    params = urlencode({"part":"snippet","type":"video","maxResults":50,"order":"viewCount","regionCode":country_code,"relevanceLanguage":language,"key":YT_KEY,"q":term})
    return get_json("https://www.googleapis.com/youtube/v3/search?"+params).get("items",[])
...
def save_channels(country, items):
    ...
    with psycopg.connect(DB) as conn:
        for item in items[:50]:
            ...
            conn.execute("""INSERT INTO tv_channels(name,youtube_channel_id,country,category) VALUES(%s,%s,%s,%s) ON CONFLICT(youtube_channel_id) DO UPDATE SET name=EXCLUDED.name,country=EXCLUDED.country,updated_at=now(),is_active=TRUE""",(name,cid,country,"Descoberta WEBPLAY")); count+=1
    return count
...
def save_videos(country,language,items,media_type,category):
    ...
    with psycopg.connect(DB) as conn:
        for item in items:
            ...
            conn.execute("""INSERT INTO media_items(title,source_name,youtube_video_id,youtube_channel_id,media_type,category,country,language,duration,thumbnail_url,source_url,is_active) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,'Completo',%s,%s,FALSE) ON CONFLICT(youtube_video_id) DO UPDATE SET title=EXCLUDED.title,source_name=EXCLUDED.source_name,youtube_channel_id=EXCLUDED.youtube_channel_id,updated_at=now()""",(title,channel,vid,cid,media_type,category,country,language,f"https://i.ytimg.com/vi/{vid}/hqdefault.jpg",f"https://www.youtube.com/watch?v={vid}"))
            conn.execute("INSERT INTO content_rights(youtube_video_id,rights_status,evidence_url) VALUES(%s,'unknown',%s) ON CONFLICT(youtube_video_id) DO NOTHING",(vid,f"https://www.youtube.com/watch?v={vid}")); count+=1
    return count

def discover_country(country,code_lang):
    code,lang=code_lang; channels=save_channels(country,youtube_search_channels(code,lang)); media=0
    for media_type,term in SEARCH_TERMS: media+=save_videos(country,lang,youtube_search_videos(code,lang,term),media_type,"Descoberta")
    return country,channels,media
